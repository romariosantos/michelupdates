(function($) {
  var buildRating = function(obj) {
    ...
    stars
      ...
      .click(function() {
        ...
        $.post(obj.url, {rating: $("a:first", this)[0].href.slice(1), id: $("a:first", this)[0].id.slice(0), id_rec : $("a", this).data('idrec') }, function(r) {
			$('.alert').html(r);
			$('.votacao').fadeIn();
		});
  ...
  var buildInterface = function(form) {
	...
	var input2  = $("#id_receita", $(form));
    var size      = optGroup.length;
	
    optGroup.each(function() {
      container.append($('<div class="star"><a id="' + input.val() + '" data-idrec="'+input2.val()+'" href="#' + this.value + '" title="Give it ' + this.value + '/'+ size +'">' + this.value + '</a></div>'));
    });
