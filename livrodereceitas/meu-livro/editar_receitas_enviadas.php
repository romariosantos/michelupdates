...
if(isset($_SESSION['receitas_user_logado']) AND isset($_SESSION['receitas_user_id'])){
	...
	if($_GET['acao'] == "editarcook"){
		...
		$editReceita = mysql_query("SELECT * FROM tb_receitas_recebidas WHERE id='$_POST[id_receita]'");
			...
			if($postado_por_user == $_SESSION['receitas_user_id']){
				...
				//print_r($_POST);
					...
					mysql_query("UPDATE tb_receitas_recebidas SET categoria = '$categoria',subcategoria='$subcategoria',nome='$nomereceita',descricao='$descricao',ingrediente='$ingrediente',modopreparo='$modopreparo',tempo_preparo='$tempo_preparo',rendimento='$rendimento' WHERE id='$idreceita'");
	...
	//exit;
		...
		$editReceita = mysql_query("SELECT * FROM tb_receitas_recebidas WHERE id='$_GET[id]'");
			while($rowR = mysql_fetch_array($editReceita)){
				$id 				= $rowR['id'];
...
?>
		...
		<div class="right_form_er">
			...
			<div class="pdd_form_er">
				...
				<div style="margin-bottom:20px;"><img src="http://www.receitinhasdavovo.com/images/tmp_receitas_recebidas/<?php echo $image;?>" width="680px" height="500px"></div>
